...
void process_positions_parallel(task2_state& state,const std::vector<u32>& positions,u32 left_token, u32 right_token,u32 merged_token, u32 best_state) {
    ...
    #pragma omp parallel
    {
        ...
        for (std::size_t idx = 0; idx<active_words.size(); ++idx) {
            ...
            for (u32 position : by_word.at(word)) {
                ...
                ev.left_position=is_live(state, left_position) ? left_position : no_position;
                ev.after_position=is_live(state, after_position) ? after_position : no_position;
                state.token[position] =merged_token;
                state.alive[right_position] = 0;
                state.next[position] = after_position;
                if (after_position != no_position) {
                ...
                state.previous[right_position]=no_position;
                state.next[right_position]= no_position;
                ev.left_token = (ev.left_position != no_position)? state.token[ev.left_position]: no_position;
                ev.after_token = (ev.after_position != no_position)? state.token[ev.after_position]: no_position;
    ...
    for (const std::vector<MergeEvent>& events : local_events) {
        for (const MergeEvent& ev : events) {
            ...
            if (ev.left_position!=no_position) {
                const u32 state_id=get_left_pair_state(state,ev.left_token,merged_token);
                add_edge(state, ev.left_position,state_id, ev.word,ev.frequency);
            }
            if (ev.after_position != no_position) {
                const u32 state_id=get_right_pair_state(state,merged_token,ev.after_token);
...
void run_merge_loop_parallel(task2_state& state) {
    ...
    for (;;) {
        ...
        if (positions.size()>= kParallelThreshold&&omp_get_max_threads() > 1) {
            process_positions_parallel(state,positions,left_token, right_token, merged_token, best_state);
        } else {
